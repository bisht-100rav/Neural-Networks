...
(train_data, train_labels), (test_data, test_labels) = data.load_data(
    num_words=10000)  # taking words that are 10000 frequent
...
def decode_review(text):  # function to print human readable review
    return " ".join([reverse_word_index.get(i, "?") for i in text])


# print(decode_review(test_data[13]))
# print(len(test_data[0]),len(test_data[1]))  #having different len of reviews
...
# saving data
model.save("model_movie_review.h5")
...
'''
model = keras.models.load_model("model_movie_review.h5")
